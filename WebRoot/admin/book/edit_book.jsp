	...
	<form class="layui-form" action="" style="display:inline-block;width:70%;margin:0 auto;padding-top:5%;">
	  ...
	  <div class="layui-form-item">
	    ...
	    <div class="layui-input-block">
	      ...
	      <input type="text" name="book_name" value="${param.book_name}" lay-verify="required" placeholder="请输入图书名称" autocomplete="off" class="layui-input">
	    </div>
	  </div>
	  <div class="layui-form-item">
	    <div class="layui-input-block">
...
layui.use(['form', 'layedit', 'laydate','layer'], function(){
  ...
  form.on('submit(demo1)', function(data){
	...
	is_ok(url,{book_id:data.field.book_id,cate_id:data.field.cate_id,book_name:data.field.book_name},"result_edit");
    return false;
  });
$(function(){
	get_book_cate();
...
function result_book_cate(data){
	...
	{  
		if(data.data[cate].cate_id == ${param.cate_id}){
			$("#book_cate").append("<option selected value='"+data.data[cate].cate_id+"'>"+data.data[cate].cate_name+"</option>");
		}else{
...
function result_edit(data){
	if(data == 1){
		layer.msg('修改图书成功', {icon: 1},function(){
		 	layer.closeAll('loading');
		 	quiet();
		 });	
	}else{
		layer.msg('修改图书失败', {icon: 2},function(){
		 	layer.closeAll('loading');
		 	quiet();
		 });	
	}
